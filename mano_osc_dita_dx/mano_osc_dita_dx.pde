...
void setup() {
  //size(800, 800, P3D);
  fullScreen(P3D,2);
  oscP5 = new OscP5(this,12001); // Start Listening on port 12000
...
void draw() {
  ...
  if (start) {
    ...
    drawDito(-80,0,-150,rotIndex/2.7,180,true);    // Indice
    drawDito(-28,0,-153,rotMiddle/2.7,200,true);   // Medio
    drawDito(24,0,-150,rotRing/2.7,180,true);      // Anulare
    drawDito(76,0,-145,rotLittle/2.7,150,true);    // Mignolo
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if(theOscMessage.checkAddrPattern("/rightGlove")==true) {
    rotxfactor  = theOscMessage.get(0).intValue(); //print("pi "); println(rotxfactor);
    rotzfactor  = theOscMessage.get(1).intValue(); //print("ro "); println(rotyfactor);
    rotyfactor  = theOscMessage.get(2).intValue(); //print("yo "); println(rotzfactor);
